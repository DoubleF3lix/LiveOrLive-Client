    ...
    return (
        <div className="flex flex-row m-1">
            ...
            <select name="playersList" id="playerSelect" className="ml-2">
                ...
                {currentActionablePlayers.map((player) => turnCount > 0 ? <option key={player.username + "_playerSelect"} value={player.username}>{player.username}</option> : <></>)}
            </select>
            ...
            <select name="itemsList" id="itemSelect" className="ml-2">
                {currentPlayer?.items.map((item) => <option key={item + "_itemSelect"} value={item}>{item}</option>)}
            </select>
