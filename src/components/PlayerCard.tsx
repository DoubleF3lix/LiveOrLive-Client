...
export default function PlayerCard({ player }: PlayerCardArgs) {
    ...
    return !player.eliminated && <div className={`flex flex-col border-3 rounded-sm my-2 p-2 md:px-4 md:py-3 ${player.inGame ? "border-foreground" : "border-muted-foreground"}`}>
        ...
        {items.length > 0 && <>
            ...
            <ul className="list-disc list-inside">
                {condenseItemList(items).map((item, index) => <li key={index + "_playerItem"}>{item.displayString}</li>)}
            </ul>
        </>}
